...
var a,b,c,d,e,f,g;f=function(a,b){var c,d,e,f,g,h,k,l,m,n,o,p,q,r,s,t;for(t=[],f=[],o=Object.keys(b),(k=0,m=o.length);k<m;k++)if(e=o[k],q=RegExp("(\\b"+e+"(?![a-zA-Z<\"=\\']))","gim"),"&amp;"===e&&(q=RegExp("("+e+")","gim")),q.test(a)){//For every possible interpretation of an acronym. i.e. 
...
return b=/!(?!(  \S)|$)/gm,a=a.replace(b,"<span class=\"invalid_exclamation\">$&</span>"),a},c=function(a){var b,c;return b=/--(?=\s)/gm,c=/\s--/gm,a=a.replace(b,"<span class=\"invalid_double_dash\">$&</span>"),a=a.replace(c,"<span class=\"invalid_double_dash\">$&</span>"),a},g=function(){var a,b;return b=$("#output").html(),a=f(b,word_acro_data),b=a.html,b=d(b),b=c(b),a.html=b,a;// returning: {'html': text_content, 'tooltipped_words':[], 'approved_acros':[]}
...
for($("#output").text($("#input-text").val()),j=g(),$("#output").html(j.html),b(j.tooltipped_words),d=j.approved_acros,h=Object.keys(d),(e=0,f=h.length);e<f;e++)c=h[e],a("#"+c,"Abbreviates to: "+word_acro_data[d[c]][0]);a(".invalid_double_dash","Error: Whitespace next to '--'"),a(".invalid_exclamation","2 spaces must appear after a '!'")})})}).call(this);
