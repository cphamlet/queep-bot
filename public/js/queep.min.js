...
var a,b,c,d,e,f,g,h;//Semi-colon must have exactly 1 space after it
f=function(a,b){var c,d,e,f,g,h,k,l,m,n,o,p,q,r,s,t;// esc = RegExp('&#x[A-F0-9]{1,4};(?!<)', "gim");
// text_content = text_content.replace esc, '<span class="unicode">$&</span>'
for(t=[],f=[],o=Object.keys(b),(k=0,m=o.length);k<m;k++)if(e=o[k],q=RegExp("(\\b"+e+"(?![a-zA-Z<\"=\\']))","gim"),"&"===e&&(q=RegExp("("+e+")","gim")),q.test(a)){//For every possible interpretation of an acronym. i.e. 
...
return b=/!(?!(  \S)| {0,}$)/gm,a=a.replace(b,"<span class=\"invalid_exclamation\">$&</span>"),a},c=function(a){var b,c;return b=/--(?=\s)/gm,c=/\s--/gm,a=a.replace(b,"<span class=\"invalid_double_dash\">$&</span>"),a=a.replace(c,"<span class=\"invalid_double_dash\">$&</span>"),a},h=function(a){var b;return b=/;(?!( \S)| {0,}$)/gm,a=a.replace(b,"<span class=\"invalid_semi_colon\">$&</span>"),a},g=function(){var a,b;return b=$("#output").text(),a=f(b,word_acro_data),b=a.html,b=d(b),b=c(b),b=h(b),a.html=b,a;// returning: {'html': text_content, 'tooltipped_words':[], 'approved_acros':[]}
...
for($("#output").text($("#input-text").val()),j=g(),$("#output").html(j.html),b(j.tooltipped_words),d=j.approved_acros,h=Object.keys(d),(e=0,f=h.length);e<f;e++)c=h[e],a("#"+c,"Abbreviates to: "+word_acro_data[d[c]][0]);a(".invalid_double_dash","Error: Whitespace next to '--'"),a(".invalid_exclamation","A '!' must have exactly 2 spaces after it"),a(".invalid_semi_colon","A ';' must have exactly 1 space after it")})})}).call(void 0);
